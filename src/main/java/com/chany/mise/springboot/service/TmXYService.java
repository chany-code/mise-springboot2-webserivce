...
public class TmXYService {
    ...
    public TmXY findXYByDong(String dong){
        String tmx ="";//여기서 변수 초기화  해야할까요? 아니면 바로 tmXYApiClient.findXYByDong(dong).getTmx(); 붙여서 선언해도 될까요?
        String tmy="";
        tmx = tmXYApiClient.findXYByDong(dong).getTmx();
        tmy = tmXYApiClient.findXYByDong(dong).getTmy();
